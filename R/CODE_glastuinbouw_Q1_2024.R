...
# gbm_meetpunten <- meetpunten %>% filter(landgebruik == "glastuinbouw" | str_detect(mp, "^GGA"))
gbm_meetpunten <- meetpunten %>% filter(`landgebruik 2015` == "Glastuinbouw" | str_detect(mp, "^GGA"))
...
reverselog_trans <- function(base = 10, n = 5) {
  ...
  trans_new(name = paste0("reverselog-", format(base)),
            transform = trans,
            inverse = inv,
            breaks = log_breaks(n = n, base = base),
            domain = c(1e-100, Inf))
}
...
data_n <- fys_chem %>%
  ...
  mutate(glas = `landgebruik 2015` == "Glastuinbouw") %>%
  filter(!is.na(glas)) %>%
  filter(datum < q_eind) %>%
...
plot_mspaf_acuut <-
  ...
  scale_y_continuous(trans = reverselog_trans(n = 8),
                     ...
                     limits = c(NA, 1), expand = expansion(c(0.05, 0)),
...
plot_mspaf_chronisch <-
  ...
  scale_y_continuous(trans = reverselog_trans(n = 8),
                     labels = scales::label_number(prefix = "1 op de ", big.mark = ".", accuracy = 1),
                     limits = c(NA, 1), expand = expansion(c(0.05, 0)),
                     sec.axis = sec_axis(trans = ~ 1 / ., name = "msPAF chronisch (%)", labels = function(x) scales::percent(x, decimal.mark = ",", accuracy = 0.01))) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
...
tabel <-
  ...
  select(Stofnaam = parnaam,
         ...
         `Hoogste PAF (chronisch)` = max_paf_chronisch) # , `Aantal keer aangetroffen` = n_aanwezig
